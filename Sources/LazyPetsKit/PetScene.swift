...
final class PetScene: SKScene {
    ...
    // MARK: - Drag to relocate

    /// Grab offset + whether the pet was already planted (timer focus/done)
    /// when the drag started, so releasing restores the right stance.
    private var dragStates: [UUID: (grabOffsetX: CGFloat, wasHolding: Bool)] = [:]

    /// The user grabbed the pet: plant it (stops a stride mid-step) and
    /// remember where on the body it was picked up, so it doesn't snap to
    /// the cursor.
    func beginDrag(id: UUID, atSceneX x: CGFloat) {
        guard let node = petNodes[id] else { return }
        dragStates[id] = (node.position.x - x, node.stateMachine.holdsPosition)
        node.stateMachine.setHoldsPosition(true)
    }

    func dragPet(id: UUID, toSceneX x: CGFloat) {
        guard let node = petNodes[id], let drag = dragStates[id] else { return }
        node.position.x = min(max(x + drag.grabOffsetX, 0), size.width)
    }

    /// Released: resume roaming, unless the pet was planted before the grab.
    func endDrag(id: UUID) {
        guard let drag = dragStates.removeValue(forKey: id) else { return }
        petNodes[id]?.stateMachine.setHoldsPosition(drag.wasHolding)
    }
