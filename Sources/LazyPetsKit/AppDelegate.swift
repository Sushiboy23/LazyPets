...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    private let boombox = BoomboxController()
    ...
    private static let boomboxPetsDefaultsKey = "boomboxPetKinds"
    private static let dialPetsDefaultsKey = "dialPetKinds"
    private static let disabledFeaturesDefaultsKey = "disabledPetFeatures"
    ...
    private static let lastPopoverTabsDefaultsKey = "petPopoverLastTabs"

    /// Last tab the user viewed in each pet's click popover, so reopening
    /// lands there instead of the leftmost feature.
    private var lastPopoverTabs: [PetKind: PetClickPopoverView.Tab] = [:]

    /// Retained while the completion chime loops so it can be stopped on time.
    private var timerDoneSound: NSSound?

    /// Plays the completion chime looped for ~3 seconds (the system "Glass"
    /// sound is only ~1s on its own). Callers gate on the "Timer sounds"
    /// setting, which remains the user's off switch.
    private func playTimerDoneSound() {
        timerDoneSound?.stop()
        guard let sound = NSSound(named: "Glass") else { return }
        sound.loops = true
        timerDoneSound = sound
        sound.play()
        DispatchQueue.main.asyncAfter(deadline: .now() + 3) { [weak self] in
            // A newer alarm may have replaced this one — only stop our own.
            guard let self, timerDoneSound === sound else { return }
            sound.stop()
            timerDoneSound = nil
        }
    }
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        rosterModel.boomboxPets = loadKinds(forKey: Self.boomboxPetsDefaultsKey) ?? []
        rosterModel.dialPets = loadKinds(forKey: Self.dialPetsDefaultsKey) ?? []
        if let raw = UserDefaults.standard.dictionary(forKey: Self.disabledFeaturesDefaultsKey) as? [String: [String]] {
            rosterModel.disabledFeatures = Dictionary(uniqueKeysWithValues:
                raw.compactMap { key, features in
                    PetKind(rawValue: key).map { ($0, Set(features.compactMap(PetFeature.init))) }
                })
        }
        rosterModel.favoriteKinds = loadKinds(forKey: Self.favoritesDefaultsKey) ?? []
        if let raw = UserDefaults.standard.dictionary(forKey: Self.lastPopoverTabsDefaultsKey) as? [String: String] {
            lastPopoverTabs = Dictionary(uniqueKeysWithValues:
                raw.compactMap { key, tab in
                    guard let kind = PetKind(rawValue: key),
                          let tab = PetClickPopoverView.Tab(rawValue: tab) else { return nil }
                    return (kind, tab)
                })
        }
    ...
    private func setUpTimers() {
        ...
        clickController.targetsProvider = { [weak self] in
            guard let self else { return [] }
            let kinds = rosterModel.activeTimerPets
                .union(rosterModel.activeTaskListPets)
                .union(rosterModel.activeBoomboxPets)
                .union(rosterModel.activeDialPets)
        ...
        // Drag a pet along the Dock to move it off a button it's covering.
        clickController.onDragBegan = { [weak self] kind, point in
            guard let self, let id = instanceIDs[kind] else { return }
            overlays.beginPetDrag(id: id, at: point)
        }
        clickController.onDragMoved = { [weak self] kind, point in
            guard let self, let id = instanceIDs[kind] else { return }
            overlays.dragPet(id: id, to: point)
        }
        clickController.onDragEnded = { [weak self] kind in
            guard let self, let id = instanceIDs[kind] else { return }
            overlays.endPetDrag(id: id)
        }
    ...
    private func petTooltip(for kind: PetKind) -> String {
        var parts: [String] = []
        if rosterModel.activeTimerPets.contains(kind) {
        ...
        if rosterModel.activeTaskListPets.contains(kind) {
            let tasks = taskStore.tasks
            parts.append(tasks.isEmpty
                ? "Task List"
                : "\(tasks.filter(\.isDone).count)/\(tasks.count) tasks done")
        }
        if rosterModel.activeBoomboxPets.contains(kind) {
            parts.append("Boombox")
        }
        if rosterModel.activeDialPets.contains(kind) {
            parts.append("Dial")
        }
    ...
    private func showPetPopover(for kind: PetKind, anchor: NSView) {
        ...
        let showsTimer = rosterModel.activeTimerPets.contains(kind)
        let showsTasks = rosterModel.activeTaskListPets.contains(kind)
        let showsBoombox = rosterModel.activeBoomboxPets.contains(kind)
        let showsDial = rosterModel.activeDialPets.contains(kind)
        let timerModel = showsTimer ? makeTimerPopoverModel(for: kind) : nil

        // Land on the active timer if there is one; otherwise the tab the
        // user last viewed (if that feature is still armed); otherwise the
        // first enabled panel in Timer → Task List → Boombox → Dial order.
        var enabledTabs: [PetClickPopoverView.Tab] = []
        if showsTimer { enabledTabs.append(.timer) }
        if showsTasks { enabledTabs.append(.tasks) }
        if showsBoombox { enabledTabs.append(.boombox) }
        if showsDial { enabledTabs.append(.dial) }
        let initialTab: PetClickPopoverView.Tab
        if timerManager.timers[kind] != nil {
            initialTab = .timer
        } else if let last = lastPopoverTabs[kind], enabledTabs.contains(last) {
            initialTab = last
        } else {
            initialTab = enabledTabs.first ?? .dial
        }
        ...
        let hosting = NSHostingController(
            rootView: PetClickPopoverView(
                ...
                boombox: showsBoombox ? boombox : nil,
                dialEnabled: showsDial,
                audioService: audioService,
                levelMonitor: levelMonitor,
                onManageTasks: { [weak self] in
                    guard let self else { return }
                    timerPopover?.performClose(nil)
                    rosterModel.pendingManagePage = .taskList
                    showManageWindow()
                },
                onTabChange: { [weak self] tab in
                    self?.setLastPopoverTab(tab, for: kind)
                },
                tab: initialTab
            )
        )
    ...
    private func setLastPopoverTab(_ tab: PetClickPopoverView.Tab, for kind: PetKind) {
        lastPopoverTabs[kind] = tab
        let raw = Dictionary(uniqueKeysWithValues:
            lastPopoverTabs.map { ($0.key.rawValue, $0.value.rawValue) })
        UserDefaults.standard.set(raw, forKey: Self.lastPopoverTabsDefaultsKey)
    }
    ...
    private func makeTimerPopoverModel(for kind: PetKind) -> TimerPopoverModel {
        ...
        model.onStart = { [weak self] seconds, note in
            guard let self else { return }
            timerNotifier.requestAuthorizationIfNeeded()
            timerManager.start(kind: kind, seconds: seconds, note: note)
            timerPopover?.performClose(nil)
        }
    ...
    private func setUpFileDrops() {
        dropController.targetsProvider = { [weak self] in
            guard let self else { return [] }
            return overlays.fileDropTargets(for: rosterModel.activeAttacksFiles)
        }
    ...
    private func wireRosterModel() {
        ...
        rosterModel.onBoomboxToggle = { [weak self] _, _ in
            guard let self else { return }
            saveKinds(rosterModel.boomboxPets, forKey: Self.boomboxPetsDefaultsKey)
        }
        rosterModel.onDialToggle = { [weak self] _, _ in
            guard let self else { return }
            saveKinds(rosterModel.dialPets, forKey: Self.dialPetsDefaultsKey)
        }
        rosterModel.onFeatureActiveChange = { [weak self] kind, feature, active in
            guard let self else { return }
            let raw = Dictionary(uniqueKeysWithValues:
                rosterModel.disabledFeatures.map { ($0.key.rawValue, $0.value.map(\.rawValue)) })
            UserDefaults.standard.set(raw, forKey: Self.disabledFeaturesDefaultsKey)
            // Disarming the timer ends any running timer, same as unassigning.
            if feature == .timer, !active {
                timerManager.cancel(kind: kind)
            }
        }
