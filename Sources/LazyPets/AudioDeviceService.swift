...
final class AudioDeviceService: ObservableObject {
    ...
    /// Volume listeners re-registered whenever the default devices change.
    private var volumeListenerAddresses: [(AudioObjectID, AudioObjectPropertyAddress)] = []
    ...
    private func reloadEverything() {
        ...
        // whichever devices are currently the defaults.
        for (objectID, address) in volumeListenerAddresses {
            var address = address
            AudioObjectRemovePropertyListenerBlock(objectID, &address, queue, listenerBlock)
        }
        ...
        for (device, scope) in [(defaultOut, kAudioObjectPropertyScopeOutput), (defaultIn, kAudioObjectPropertyScopeInput)] {
            ...
            volumeListenerAddresses.append((device, address))
        }
